    ...
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="bg-medium text-dark flex items-center justify-between rounded">
        <div class="flex justify-between flex-1">
            ...
            @if ($paginator->hasMorePages())
                <a href="{{ $paginator->nextPageUrl() }}" class="relative inline-flex items-center px-4 py-2 ml-3 text-sm font-medium text-dark bg-light border border-light leading-5 rounded-md hover-medium focus:outline-none focus:ring ring-light ring-inset focus-border-light active-bg-light active-dark transition ease-in-out duration-150">
                    {!! __('pagination.next') !!}
                </a>
            @else
                <span class="relative inline-flex items-center px-4 py-2 ml-3 text-sm font-medium text-dark bg-light border border-light cursor-default leading-5 rounded-md">
                    {!! __('pagination.next') !!}
                </span>
            @endif
        </div>
        ...
        <div class=" sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
                <p class="text-sm text-dark leading-5 min-[950px]:hidden font-bold font pl-40 pr-30 min-[600px]:max-[800px]:pl-20 max-[600px]:pl-4">
                    {!! __('Showing') !!}
